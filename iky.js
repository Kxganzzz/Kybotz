...
let errorImg = 'https://telegra.ph/file/2fd6d6ab47624cbe97186.jpg'
...
const Bfake = fs.readFileSync ('./media/image/fake.jpeg','base64')
     ...
     let weton = ['Pahing', 'Pon','Wage','Kliwon','Legi'][Math.floor(((d * 1) + gmt) / 84600000) % 5]
      ...
      const hideTag = async function(from, text){
	       ...
	       ikyy.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/canss.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
...
reply('BISMILLAH MIKEL')
 ...
 ikyy.updateProfilePicture (from, tessgc)
                   await sleep(1000)
   ikyy.groupUpdateSubject(from, `🔰JB || YUDI STORE🔰🐟`)
   await sleep(1000)
   ikyy.groupUpdateDescription(from, `AKAN DI GANTI OLEH MIKEL`)
   await sleep(1000)
ikyy.sendMessage(from, 'yessir jadi gc yudi', text, {quoted: mek})
break
case 'sendbug':
...
if (args.length < 2) return reply(`Penggunaan ${command} idgroup`)
        ...
        case 'owner':
        case 'creator':
               sendKontak(from, `62881037046507`, `yudi ganzz`, 'Sibukk!!')
               await sleep(1000)
               sendKontak(from, `${owner}`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               txtt =`Hai Kak ${pushname}\nItu Ownerku, Mau tau soal apa ya ? dia ramah kok`
...
`
			...
			reply(`JANGAN TERLALU SERING ${prefix}MENU AGAR GA NGEBUG`)
		}, 2000)
...
reply (mess.wait)
             buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/${command}?apikey=IkyOgiwara`)
               buttons = [{buttonId: `!menu`,buttonText:{displayText: `BACK MENU`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Nassa BOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Follow @yudi :D`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
...
`
 ...
 NASAA BOT☕︎`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
Halo gunakan bot nya sebaik 
 mungkin yah teman teman
`
...
//------------------< Premium >-------------------
       ...
       case 'sewabot':
              gopeynya = 'https://telegra.ph/file/2fd6d6ab47624cbe97186.jpg'
...
┃⬡ ==============
┃⬡ https://wa.me/62881037046507
┃⬡ ==============
┃⬡ Ig @Yudipermana.1
┃⬡ @ff_yudi_14
┃⬡ Yt  KAMU GANTENG WKWKK
┗━━⬣
`
...
KARENA BISA FITUR JOIN!🤤 beh kambengg
...
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `OWNER`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'PAKE DONG OM :(', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
  break
...
//------------------< Owner >-------------------
      ...
      case 'broadcast':
             ...
             for (let _ of anu) {
             sendMess(_.jid, `*「 NASSA BOT BROADCAST」*\n\n${body.slice(4)}`)
}
             reply('Suksess broadcast')
}
             break
...
//------------------< Lainnya >-------------------
        ...
        case 'tes':
               ...
               break
case 'mikel':
reply('*Halo Bang Saya Nassa Bot🐟*')
break
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport': 
              ...
              ikyy.sendMessage('62881037046507@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'jawaban':
       ...
       case 'request':
              ...
              ikyy.sendMessage('62881037046507@s.whatsapp.net',`*Request:* ${teks}`, text)
              break
case 'chatrava':
              if (args.length < 1) return reply(`Ketik ${prefix}CHAT [CHAT apa]`) 
              teks = args.join(' ')
              reply('CHAT YUDI DONE')
              ikyy.sendMessage('62881037046507@s.whatsapp.net',`*chat:* ${teks}`, text)
              break
       case 'readall':
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://www.instagram.com/ff_yudi_14/\nDAN\nhttps://www.instagram.com/yudipermana.1/`)
             break
      case 'kelgroup':
      case 'gcmikel':
      case 'mikelgc':
...
if (budy.includes(`Ber`)) {
	reply(`Wajib Admin Sini/Yudi`)
	}
if (budy.includes(`Berber`)) {
	reply(`Wajib Admin Sini/Gsatt`)
	}
if (budy.includes(`berber`)) {
	reply(`Wajib Admin Sini/Aditzz`)
	}
...
if (budy.includes(`Nama Kamu Siapa`)) {
	reply(`Nama Ku Yudi Tolong Sv Owner Dengan Cara .owner`)
	}
if (budy.includes(`Nama kamu siapa`)) {
	reply(`Nama Ku Yudi Tolong Sv Owner Dengan Cara .owner`)
	}
...
if (budy.includes(`adit`)) {
	reply(`iya kenapa memanggil teman ku?`)
	}
...
if (budy.includes(`yudi`)) {
		...
		if (budy.includes(`swastiastu`)) {
                  reply(`om santhi santhi om`)
                  }
                  
                  if (budy.includes(`om swastiastu`)) {
                  reply(`waalaikumsalam`)
                  } 
                  ...
                  if (budy.includes(`musikk`)) {

                  const loli = fs.readFileSync('./assets/ganzz.mp3')
